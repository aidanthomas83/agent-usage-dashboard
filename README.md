...
Skill/plugin detection depends on what the Codex client persisted into the rollout. Calls routed through programmatic/code-mode orchestration are detected where their persisted tool code contains recognisable `mcp__...` or `skills://.../skill.md` references, but the view can still be incomplete.
